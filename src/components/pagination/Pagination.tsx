...
const PaginationButtons: React.FC<PaginationButtonsProps> = ({ pageIndex, totalPage, onPageChange }) => {
  return (
    <div className="flex justify-center px-4 py-2 rounded-xl mt-auto">
      <ul className="flex items-center">
        ...
        <li className="px-[6px]">
          <span className="p-2 rounded-lg text-base text-white bg-blue-500 hover:cursor-default">
            {pageIndex === 0 ? 10 : (((pageIndex + 1) * 10) < totalPage ? ((pageIndex + 1) * 10) : pageIndex * 10)} de {totalPage}
          </span>
        </li>
        <li className="px-[6px]">
          <button
        ...
        <li className="px-[6px]">
          <button
            ...
            disabled={totalPage === pageIndex + 1}
            onClick={() =>
              onPageChange((Math.ceil(totalPage / 10)) - 1)
            }
          >
